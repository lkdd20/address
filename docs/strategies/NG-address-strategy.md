...
| 项目 | 核心内容 |
|---|---|
| 数据源 | Geofabrik `nigeria`；Google Geocoding 补全 |
| 格式 / 邮编 | `<house> <street>, <district>, <city>, <state> <postcode>, NG`；仅保留源记录且通过 6 位格式门禁；任一必填组件缺失即淘汰，不推算。只清洗格式，不创造或按邻近地址补齐 |
| 行政区 | 源字段与 catalog；官方行政边界作为候选核验。行政层级必须与坐标反查一致；冲突时拒绝发布 |
| 住宅证据 | OSM 明确住宅建筑标签；Google 只补全地址字段，不单独证明住宅 |
| 坐标 / 去重 | WGS84 经纬度并通过尼日利亚边界门禁；同一批次按规范化地址 canonical hash 去重 |
| 发布门禁 | 质量高于数量。仅发布 E3：地址存在与独立住宅用途证据同时成立；本国格式规定的必填组件缺一即淘汰，字段冲突拒绝合并；只允许可逆、可验证的格式规范化。 |
| 同步方式 / 频率 | Geofabrik 版本检查；Google 补全使用可恢复 checkpoint、额度冷却和自动耗尽判定 |
| 验证 / 排除 | 必须同时具备来源 ID、门牌、街道、district、城市、州、六位邮编、坐标和 OSM 住宅建筑证据；任一缺失即拒绝 |
| 策略版本 / 状态 / 更新 | 1.7 / Geofabrik + Google 补全 / 2026-08-24 |

- 默认 active 地址上限 8,000；州、地方政府区、地区单节点上限 1,000/200/50，后台可覆盖；只裁剪地址记录，行政区划与邮编目录保持完整。
