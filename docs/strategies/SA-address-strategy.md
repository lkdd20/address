...
| 项目 | 核心内容 |
|---|---|
| 主源 | Geofabrik `gcc-states` 的 SAU 边界分片；可选全国地址点保全包；Google Geocoding 补全 |
...
| 验证 / 排除 | 全国地址点和地理编码结果必须精确关联到明确住宅建筑面；按 SAU 边界提取，避免邻国记录进入地址池 |
| 策略版本 / 状态 / 更新 | 1.4 / 全国候选 + 严格住宅点面关联 + 多规则完成下限 / 2026-08-24 |
...
- 同步：配置 Google Geocoding 凭据后按有界批次执行。阿拉伯-印度数字门牌和邮编等值规范为 ASCII 数字后再执行邮编门禁。checkpoint 按来源和适配器指纹隔离；连续批次无进展时标记来源已达上限，凭据或额度等待不会误判为耗尽。
