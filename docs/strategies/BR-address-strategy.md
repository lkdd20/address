...
| 真实字段 / 生成字段 | 门牌、道路、neighborhood、城市、UF、CEP 和坐标均来自 OSM；生成字段：无，只保留来源缩写与重音 |
| 坐标 / 去重 | OSM WGS84 坐标通过巴西边界门禁；按 OSM ID 和规范化地址 canonical hash 去重 |
| 发布门禁 | 质量高于数量。仅发布 E3：地址存在与独立住宅用途证据同时成立；本国格式规定的必填组件缺一即淘汰，字段冲突拒绝合并；只允许可逆、可验证的格式规范化。 |
| 同步方式 | 自动初始化一次；checkpoint 自动续跑，完整扫描或来源耗尽后停止。显式来源探测发现 Geofabrik 版本变化时才构建新快照 |
| 验证 / 排除 | UF 必须与行政目录一致；葡萄牙语重音保留；缺少完整地址、坐标或住宅建筑证据的记录拒绝发布 |
