...
| 真实字段 / 生成字段 | 门牌、道路、locality、联邦主体、邮编、可选 корпус/квартира 和坐标来自 OSM；生成字段：无 |
| 坐标 / 去重 | OSM WGS84 坐标通过俄罗斯边界与脚本门禁；按 OSM ID 和规范化西里尔文地址 canonical hash 去重 |
...
| 验证 / 排除 | 俄语行政层级不能从街道名猜测；跨境、缺字段和脚本异常记录直接拒绝 |
| 策略版本 / 状态 / 更新 | 1.3 / 严格住宅点面关联 + 多规则完成下限 / 2026-08-24 |

- 默认 active 地址上限 20,000；联邦主体、市/区、地区单节点上限 2,000/300/70，后台可覆盖；只裁剪地址记录，行政区划与邮编目录保持完整。
